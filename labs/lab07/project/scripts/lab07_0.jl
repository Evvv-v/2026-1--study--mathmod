...
# исходные данные для собственного примера
N = 1200.0          # общее число потенциальных клиентов
n0 = 8.0            # число клиентов, знающих о салоне в начальный момент
u0 = [n0]
tspan = (0.0, 30.0)

# общая модель распространения рекламы:
# dn/dt = (alpha1(t) + alpha2(t) * n(t)) * (N - n(t))
...
# случай 1: платная реклама сильнее сарафанного радио, alpha1(t) > alpha2(t)
alpha1_case1(t) = 0.07
...
# случай 2: сарафанное радио эффективнее платной рекламы, alpha1(t) < alpha2(t)
...
# модель только платной рекламы: alpha2(t) = 0
alpha1_paid(t) = 0.06
alpha2_paid(t) = 0.0
...
# модель только сарафанного радио: alpha1(t) = 0
alpha1_word(t) = 0.0
alpha2_word(t) = 0.004
...
# график сравнения трех вариантов рекламной кампании
p1 = plot(sol_case1,
...
# сравнение платной рекламы и сарафанного радио
p4 = plot(sol_paid,
