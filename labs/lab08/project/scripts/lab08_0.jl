...
# активируем проект, если скрипт лежит внутри lab08/project или lab08/project/scripts
...
# Общее задание: свой пример двух фирм с идентичным товаром
# ================================================================
...
p1 = 9.0         # себестоимость товара фирмы 1, тыс. ед.
p2 = 7.0         # себестоимость товара фирмы 2, тыс. ед.
N = 10.0         # число потребителей, тыс. ед.
q = 1.0          # максимальная потребность одного человека

# начальные оборотные средства, млн. ед.
M0 = [2.0, 1.0]
tspan = (0.0, 30.0)

# коэффициенты модели
# b берется в соответствии с формулой методички: с произведением квадратов tau и p
# иначе стационарное состояние получается некорректным
...
# социально-психологический коэффициент для случая 2
extra_factor = 0.002

# стационарное состояние для первого случая:
# a1*M1 + b*M2 = c1
# b*M1 + a2*M2 = c2
...
println("Стационарное состояние для случая 1:")
...
# случай 2: учитывается социально-психологический фактор
...
plt2 = plot(sol2, vars = (0, 1),
    label = "Фирма 1",
    lw = 2,
    title = "Случай 2: социально-психологический фактор",
    xlabel = "Безразмерное время θ",
    ylabel = "Оборотные средства M")
...
save_path = joinpath(plots_dir, "lab08_general_competition.png")
