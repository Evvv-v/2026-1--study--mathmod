...
# ## Вариант 54
# ## Задача №2: осциллятор с затуханием и без внешней силы
# Уравнение варианта: x'' + 13x' + 13x = 0
# Начальные условия: x(0)=0.9, y(0)=0.9
# Интервал: t ∈ [0; 48], шаг 0.05

# ## 1. Параметры модели
stiffness = 13.0          # коэффициент при x
damping = 13.0            # коэффициент при x'
u0 = [0.9, 0.9]
tspan = (0.0, 48.0)
step = 0.05
...
# x' = y
# y' = -13y - 13x
function variant54_with_damping!(du, u, p, t)
    x, y = u
    du[1] = y
    du[2] = -damping * y - stiffness * x
end

# ## 3. Решение системы
prob = ODEProblem(variant54_with_damping!, u0, tspan)
sol = solve(prob, Tsit5(), saveat=step, abstol=1e-8, reltol=1e-8)

# ## 4. Построение решения и фазового портрета
p_solution = plot(sol,
    title="Вариант 54: с затуханием",
    xlabel="t",
    ylabel="x(t), y(t)",
    label=["x(t)" "y(t)"],
    lw=2
...
save_to_existing_plots(result_plot, "lab04_4_variant54_with_damping.png")
