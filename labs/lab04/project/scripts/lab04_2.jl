...
# ## Задача №3: гармонический осциллятор с затуханием и внешней силой
# Уравнение: x'' + 2γ*x' + w0^2*x = f(t)
...
function oscillator_with_force!(du, u, p, t)
    x, y = u
    du[1] = y
    du[2] = f_ext(t) - 2 * gamma_val * y - w0^2 * x
end

# ## 3. Решение системы
prob = ODEProblem(oscillator_with_force!, u0, tspan)
sol = solve(prob, Tsit5(), saveat=step)

# ## 4. Построение решения и фазового портрета
p_solution = plot(sol,
    title="Решение: с внешней силой",
    xlabel="t",
    ylabel="x(t), y(t)",
    label=["x(t)" "y(t)"],
    lw=2
...
save_to_existing_plots(result_plot, "lab04_2_with_force.png")
